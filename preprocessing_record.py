...
def preprocess(channel_data, sampling_rates, preprocessing_sampling_rate):
    try:
        ...
        data: np.ndarray = data_new.reshape([n_channels, -1, preprocessing_sampling_rate * 30]).transpose(
            [1, 0, 2]).astype(
            'float32')
        del data_new
        ...
        print(e)
        return True, []
    return False, data.tolist()


# params are set in global context by webworker
error, samples = preprocess(channel_data, sampling_rates, preprocessing_sampling_rate)
